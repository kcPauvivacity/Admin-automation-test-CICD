...
test.describe('Enquiries Tests', () => {
    test.beforeEach(async ({ page }) => {
        await loginToApp(page);
        await page.waitForLoadState('load');
    });

    test('should access Enquiries page', async ({ page }) => {
    ...
    test('should display Enquiries list/table', async ({ page }) => {
        test.setTimeout(60000);
        
        const healer = createAutoHealing(page);
        
        // Navigate to Enquiries
        await healer.smartClick(['text=Enquiries']);
        await page.waitForLoadState('load');
        await healer.waitForStable();
        ...
        console.log(`Found ${rows} rows/items in Enquiries`);
    ...
    test('should navigate through Enquiries sections', async ({ page }) => {
        test.setTimeout(60000);
        ...
        if (tabCount > 0) {
            console.log(`Found ${tabCount} tabs in Enquiries`);
        ...
        console.log('✅ Enquiries navigation tested');
    });
