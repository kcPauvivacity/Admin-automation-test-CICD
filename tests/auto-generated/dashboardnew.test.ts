...
test.describe('Dashboard Module Tests', () => {
    test.beforeEach(async ({ page }) => {
        ...
        const menuItem = page.getByRole('menuitem', { name: /Dashboard/i });
    ...
    test('should load Dashboard page successfully', async ({ page }) => {
        ...
        const url = page.url();
        const urlPath = 'dashboardnew'.replace('new', ''); // Handle 'New' suffix
    ...
    test('should display Dashboard table with data', async ({ page }) => {
        test.setTimeout(60000);
        ...
        if (hasTable) {
            ...
            console.log(`📊 Found ${rowCount} rows in Dashboard table`);
            ...
            if (headerCount > 0) {
                console.log(`✅ Dashboard table displays correctly with ${headerCount} columns`);
            } else {
                console.log(`ℹ️ Dashboard table has no headers (might be a different layout)`);
            }
        } else {
            console.log(`ℹ️ No standard table found - Dashboard might use a different data display format`);
        }
    });

    test('should handle Dashboard table pagination', async ({ page }) => {
        test.setTimeout(60000);
    ...
    test('should verify Dashboard action buttons', async ({ page }) => {
        test.setTimeout(60000);
        ...
        const expectedButtons = ['Overview', 'Enquiry Analytics', 'Mini App Usage', 'Demographics'];
        ...
        console.log('✅ Dashboard buttons verified');
    });
    ...
    test('should navigate through Dashboard sections', async ({ page }) => {
        ...
        if (tabCount > 0) {
            console.log(`📑 Found ${tabCount} tabs in Dashboard`);
            ...
            console.log('ℹ️ No tabs found in Dashboard');
        }
